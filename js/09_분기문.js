...
function startGame(){
  ...
  // Math.random() -> 0.0 ~ 1.0 사이의 난수 발생 (1.0 미만)
  ...
  while(true){  // 무한 반복
    ...
    // - 미입력한 경우 : alert("다시 입력해주세요") - (카운트 증가 X)
    if(input.length === 0){
      alert("다시 입력해주세요");
    ...
    // 매번 prompt에 내가 작성했었던 input 값을 쌓아가면서 보여주기
    ...
    // 입력 받은 수가 난수(answer) 보다 높거나 낮을 경우
    if(input > answer){
      alert(`${input} DOWN (현재 ${count})회`);
    }else{
      alert(`${input} UP (현재 ${count})회`);
    }
