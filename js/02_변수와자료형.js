...
function varCheck(){
  ...
  // var 사용의 문제점
  // -> 잘 사용하고 있던 변수를 실수로 없애는 경우 자꾸 발생
}
...
function letCheck(){
  let name = "김영수";  /* 변수 선언 + "김영수" 대입 */

  console.log("name : ", name); // 김영수
...
{ // 함수 정의 시작
  ...
  // if : 만약에
  // if(num === 100) : 만약에 num1과 100이 같다면 {}블럭 수행
  if(num1 === 100){
...
function typeCheck(){
  ...
  // sumFm 호출 -> () 내 값 전달 -> 계산 결과 return -> console 출력
  console.log('sumFn 호출 결과 : ', sumFn(111, 89), typeof sumFn(111, 89));
  console.log('sumFn 호출 결과 : ', sumFn(3.3, 5.1), typeof sumFn(3.3, 5.1));
  console.log('sumFn 호출 결과 : ', sumFn(-111, 89), typeof sumFn(-111, 89));
