...
/* 버튼 배경색 변경 + 클릭 시 #result2 배경색 변경경 */
...
/* 
  ...
  바로 위 .display 요소의 배경색 변경하기기
*/

const change = document.querySelector("#changeBtn");
const getColor = document.querySelectorAll(".inputColor");
const result = document.querySelectorAll(".display");

for(let i=0; i < getColor.length; i++){

change.addEventListener('click', function(){
  // inputColor에 얻어온 값을 display 요소의 배경색에 변경시킨다다
  result[i].style.backgroundColor = getColor[i].value;
})
}