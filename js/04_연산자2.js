...
function check1(){
  ...
  // number, string 동일 비교
  console.log('num1 == str3 : ', num1 == str3); // false
  // -> 왜? 자료형이 다름
...
function orCheck(){
  // 4는 10을 초과하거나 짝수인가?
  const jjack = (4 > 10) || (4 % 2 === 0);
  console.log(`4는 10을 초과하거나 짝수인가? ${jjack}`);
  
  
  // 나이가 18세 미만 이거나, 이름이 "홍길동" 인가?
  const age = 20;
  const name = '홍길동';
  const result = (age < 18) || (name === '홍길동');
  console.log(`나이가 18세 미만 이거나, 이름이 "홍길동" 인가? ${result}`);
  ...
  const kor = 50;
  const eng = 75;
  const avg = ((kor < 40) || (eng <40)) || ((kor+eng)/2 < 60);
  console.log(`국어 또는 영어 점수가 40점 미만이거나 평균이 60점 미만인가? ${avg}`);
